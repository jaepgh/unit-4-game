...
function attackEnemy() {
    ...
    if (!game.enemySelected.isAlive()) {
        ...
        game.playerSelected.updateCurrentLifePoints(game.enemySelected.currentAttackPower);
        updatePlayerStats();
...
function hideDefeatedCharacter() {
    $("#column-" + game.enemySelected.playerId).css("display", "none");
}
...
function showControls() {
    ...
    $("#Player" + (game.playerSelected.playerId + 1) + "Stats").css("display", "block");

    //show progress defender
    $("#Player" + (game.enemySelected.playerId + 1) + "Stats").css("display", "block");
}
...
function updatePlayerStats() {
    //Update attacker
    $("#hp-progress-" + game.playerSelected.playerId).css("width", game.playerSelected.lifePercentage() + "%");
    $("#ap-progress-" + game.playerSelected.playerId).css("width", game.playerSelected.attackerPowerPercentage() + "%");

    //Update defender
    $("#hp-progress-" + game.enemySelected.playerId).css("width", game.enemySelected.lifePercentage() + "%");
...
function updateMessages(enemyDamage, playerDamage) {
    ...
    $("#attacker").text("You attacked " + game.enemySelected.playerName + " for " + playerDamage + " damage.");
    $("#defender").text(game.enemySelected.playerName + " attacked you back for " + enemyDamage + " damage.");
}

function gameOver() {
    $("#presentation").text("You have been defeated by " + game.enemySelected.playerName + ". GAME OVER!!!");
    hideControls();
    showReStart();
}
...
function moveToNextPlayer() {
    hideControls();
    $("#presentation").text("You defeated " + game.enemySelected.playerName + ". Select your next opponent!");
...
function resetGameControls() {
    hideReStart();
    hideControls();
    $("#presentation").text("Select you caracter!");
    showAvaliableCharacters();
